...
(function(){
  ...
  var PAGE_SIZE=50;
  var historyState={threadId:null,oldest:null,hasOlder:false};
  ...
  if(typeof baseOpen==='function'){
    window._mOpenConv=async function(threadId,otherId,otherName){
      historyState={threadId:threadId,oldest:null,hasOlder:false};
  ...
  async function messageNode(m,me){
    var mine=m.sender_id===me,outer=n('div',null,'display:flex;flex-direction:column;align-items:'+(mine?'flex-end':'flex-start'));outer.dataset.messageId=m.id;
    ...
    if(!mine)meta.appendChild(b('Signaler',function(){reportMessage(m.id);},'border:none;background:transparent;color:var(--c-text-4);padding:0;font-size:9.5px;text-decoration:underline;cursor:pointer'));
    outer.appendChild(meta);return outer;
  }

  async function fetchPage(threadId,before){
    var q=SupaDB.from('messages').select('id,sender_id,contenu,created_at,attachment_url,attachment_type').eq('thread_id',threadId).eq('is_deleted',false).order('created_at',{ascending:false}).limit(PAGE_SIZE+1);
    if(before)q=q.lt('created_at',before);
    var r=await q;if(r.error)return r;
    var rows=r.data||[],hasOlder=rows.length>PAGE_SIZE;if(hasOlder)rows=rows.slice(0,PAGE_SIZE);rows.reverse();
    return {data:rows,error:null,hasOlder:hasOlder};
  }

  async function prependOlder(threadId){
    var c=document.getElementById('msg-list');if(!c||historyState.threadId!==threadId||!historyState.oldest)return;
    var load=document.getElementById('msg-load-older');if(load)load.disabled=true;
    var beforeHeight=c.scrollHeight,beforeTop=c.scrollTop,r=await fetchPage(threadId,historyState.oldest),me=await uid();
    if(load)load.disabled=false;if(r.error){toast('Impossible de charger l’historique.','#C62828');return;}
    var rows=r.data||[];if(!rows.length){historyState.hasOlder=false;if(load)load.remove();return;}
    var frag=document.createDocumentFragment();for(var i=0;i<rows.length;i++)frag.appendChild(await messageNode(rows[i],me));
    var anchor=document.getElementById('msg-load-older-wrap');if(anchor)anchor.after(frag);else c.prepend(frag);
    historyState.oldest=rows[0].created_at;historyState.hasOlder=r.hasOlder;
    if(!r.hasOlder&&load)load.remove();c.scrollTop=beforeTop+(c.scrollHeight-beforeHeight);
  }

  window._mLoadMessages=async function(threadId){
    var c=document.getElementById('msg-list');if(!c||!window.SupaDB)return;
    historyState={threadId:threadId,oldest:null,hasOlder:false};
    var r=await fetchPage(threadId,null),me=await uid();clear(c);
    ...
    var msgs=r.data||[];if(!msgs.length){c.appendChild(n('div','Aucun message. Dites bonjour !','text-align:center;color:var(--c-text-3);font-size:12px;padding:20px'));return;}
    historyState.oldest=msgs[0].created_at;historyState.hasOlder=r.hasOlder;
